...
with open(output_file, 'w') as write_file:
    ...
    with open('files/nft.csv', 'r') as csv_file:
        ...
        for row in data:
            if row[1] and row[2]:
                sn, file_name, name, description, gender, trait_types, uuid = row[0], row[1], row[2], row[3], row[4], row[5].split(', '), row[-1]
                name_trait = trait_types[0].split(':')
                ...
                #weakness_trait = trait_types[7].split(':')
